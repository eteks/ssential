...
class ControllerMagikblogCategory extends Controller {
	...
	public function add() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			...
			$this->response->redirect($this->url->link('magikblog/category', 'token=' . $this->session->data['token'], true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('magikblog/category', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('magikblog/category', 'token=' . $this->session->data['token'] . $url, true)
		);
		
		$data['add'] = $this->url->link('magikblog/category/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('magikblog/category/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['blogcategories'][] = array(
				...
				'edit'       => $this->url->link('magikblog/category/edit', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'] . $url, true)
		...
		$data['sort_name'] = $this->url->link('magikblog/category', 'token=' . $this->session->data['token'] . '&sort=r.name' . $url, true);
		$data['sort_status'] = $this->url->link('magikblog/category', 'token=' . $this->session->data['token'] . '&sort=r.status' . $url, true);
		$data['sort_display_order'] = $this->url->link('magikblog/category', 'token=' . $this->session->data['token'] . '&sort=r.display_order' . $url, true);
		...
		$pagination->url = $this->url->link('magikblog/category', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('magikblog/category', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['category_id'])) {
			$data['action'] = $this->url->link('magikblog/category/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('magikblog/category/edit', 'token=' . $this->session->data['token'] . '&category_id=' . $this->request->get['category_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('magikblog/category', 'token=' . $this->session->data['token'] . $url, true);

		$this->load->model('magikblog/category');

		if (isset($this->request->get['category_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) { 
			$category_info = $this->model_magikblog_category->getBlogcategory($this->request->get['category_id']);
			$category__keyword_info = $this->model_magikblog_category->getBlogcategorykeyword($this->request->get['category_id']);
		}
		...
		$this->response->setOutput($this->load->view('magikblog/category_form', $data));
	}
	...
	protected function validateForm() {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			if ($url_alias_info && isset($this->request->get['category_id']) && $url_alias_info['query'] != 'mgkblogcategory_id=' . $this->request->get['category_id']) {
