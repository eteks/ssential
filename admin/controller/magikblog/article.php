...
class ControllerMagikblogArticle extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			...
			$this->response->redirect($this->url->link('magikblog/article', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('magikblog/article', 'token=' . $this->session->data['token'] . $url, true)
		);
		
		$data['add'] = $this->url->link('magikblog/article/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('magikblog/article/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['articles'][] = array(
				...
				'edit'       => $this->url->link('magikblog/article/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $result['blog_id'] . $url, true)
...
//print_r($data['articles']);exit();
		...
		$data['sort_name'] = $this->url->link('magikblog/article', 'token=' . $this->session->data['token'] . '&sort=md.name' . $url, true);
		$data['sort_status'] = $this->url->link('magikblog/article', 'token=' . $this->session->data['token'] . '&sort=m.status' . $url, true);
		...
		$pagination->url = $this->url->link('magikblog/article', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('magikblog/article', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['blog_id'])) {
			$data['action'] = $this->url->link('magikblog/article/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('magikblog/article/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $this->request->get['blog_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('magikblog/article', 'token=' . $this->session->data['token'] . $url, true);
		...
		if (isset($this->request->get['blog_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			$article_keyword_info = $this->model_magikblog_article->getBlogarticlekeyword($this->request->get['blog_id']);
		}
		...
		if (isset($this->request->post['related_product'])) {
			...
			$products = $this->model_magikblog_article->getRelatedProduct($this->request->get['blog_id']);
...
// related articles
		if (isset($this->request->post['related_article'])) {
			...
			$related_article = $this->model_magikblog_article->getRelatedArticle($this->request->get['blog_id']);
		...
		$related_article_info = $this->model_magikblog_article->getarticleDescriptions($article_id);
		...
		$this->response->setOutput($this->load->view('magikblog/article_form', $data));
	}
	public function add() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			...
			$this->response->redirect($this->url->link('magikblog/article', 'token=' . $this->session->data['token'], true));
		}
	...
	protected function validateForm() {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			if ($url_alias_info && isset($this->request->get['blog_id']) && $url_alias_info['query'] != 'mgkblogarticle_id=' . $this->request->get['blog_id']) {
