...
class ControllerExtensionTotalCodOrderFeeTotal extends Controller {
	...
	public function index() {
		$this->load->language('total/cod_order_fee_total');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('cod_order_fee_total', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/total/cod_order_fee_total', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/total/cod_order_fee_total', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=total', true);

		if (isset($this->request->post['cod_order_fee_total_status'])) {
			$data['cod_order_fee_total_status'] = $this->request->post['cod_order_fee_total_status'];
		} else {
			$data['cod_order_fee_total_status'] = $this->config->get('cod_order_fee_total_status');
		}

		if (isset($this->request->post['cod_order_fee_total_sort_order'])) {
			$data['cod_order_fee_total_sort_order'] = $this->request->post['cod_order_fee_total_sort_order'];
		} else {
			$data['cod_order_fee_total_sort_order'] = $this->config->get('cod_order_fee_total_sort_order');
		}
