	...
	<div class="container-fluid">
		<form class="form-horizontal autosave">
			...
			<?php foreach ($settings as $setting) { ?>
				...
				<?php $key = (isset($setting['key']) ? $setting['key'] : ''); ?>
				...
				$all_attributes = 'class="form-control" name="' . $key . '" id="input-' . $key . '" ' . ($help_text ? "data-help='" . $help_text . "' " : '') . $attributes;
				?>
				
				
				<?php if (isset(${'entry_'.$key}) || isset($setting['title'])) { ?>
					<div class="form-group">
						<label class="control-label col-sm-3" for="input-<?php echo $key; ?>"><?php echo (isset($setting['title'])) ? $setting['title'] : ${'entry_'.$key}; ?></label>
						<div class="col-sm-9">
				<?php } else { ?>
					<div class="setting">
				<?php } ?>
				...
				<?php if ($setting['type'] == 'button') { ?>
					...
					<?php } elseif ($key == 'delete') { ?>
						<a class="btn btn-danger" data-help='<?php echo $text_delete; ?>' onclick="if (confirm('<?php echo $standard_confirm; ?>')) removeRow($(this).parents('tr'))"><i class="fa fa-trash-o fa-lg fa-fw"></i></a>
					<?php } ?>
				...
				<?php } elseif ($setting['type'] == 'checkboxes') { ?>
					...
					<?php foreach ($setting['options'] as $val => $text) { ?>
						<label><input type="checkbox" class="form-control" name="<?php echo $key; ?>" <?php echo $attributes; ?> <?php if (in_array($val, $values)) echo 'checked="checked"'; ?> value="<?php echo $val; ?>" /> <span <?php if ($help_text) echo "data-help='" . $help_text . "'"; ?>><?php echo $text; ?></span></label>
					<?php } ?>
				...
				<?php } elseif ($setting['type'] == 'multilingual_text') { ?>
					...
					<?php if (!empty($setting['admin_ref'])) { ?>
						...
						<div class="input-group" data-help='<?php echo $help_admin_reference; ?>'>
							...
							<input type="text" class="form-control" placeholder="<?php echo $text_admin_reference; ?>" name="<?php echo $key; ?>_admin" <?php echo $attributes; ?> value='<?php echo str_replace("'", "\'", $value); ?>' />
						</div>
					<?php } ?>
					<?php foreach ($language_array as $language) { ?>
						<?php $value = (isset($saved[$key.'_'.$language['code']])) ? $saved[$key.'_'.$language['code']] : (isset($setting['default']) ? $setting['default'] : ''); ?>
						<div class="input-group" <?php if ($help_text) echo "data-help='" . $help_text . ' ' . $language['name'] . "'"; ?>>
							<span class="input-group-addon"><img src="view/image/flags/<?php echo $language['image']; ?>" /></span>
							<input type="text" class="form-control" placeholder="<?php if (isset(${'placeholder_'.$help_key})) echo $language['name'] . ' ' . ${'placeholder_'.$help_key}; ?>" name="<?php echo $key . '_' . $language['code']; ?>" <?php echo $attributes; ?> value='<?php echo str_replace("'", "\'", $value); ?>' />
						</div>
					<?php } ?>
					
				<?php } elseif ($setting['type'] == 'textarea') { ?>
					
					</div><textarea <?php echo $all_attributes; ?>><?php echo $value; ?></textarea><div>
				...
				<?php } elseif ($setting['type'] == 'rule') { ?>
					...
					<?php foreach (array_merge(array(0), $setting['rules']) as $rule) { ?>
						...
						<div class="rule" <?php if ($rule == 0) echo 'style="display: none"'; ?>>
							...
							<a class="btn btn-danger btn-xs" data-help='<?php echo $text_delete; ?>' onclick="removeRow($(this).parent())"><i class="fa fa-trash-o fa-lg"></i></a>
							...
							<?php foreach (array('max', 'min', 'shipping_cost') as $criterion) { ?>
								<?php if (!in_array($criterion, array_merge($rule_options['adjustments'], $rule_options['order_criteria']))) continue; ?>
								<?php if ($type == 'shipping' && $criterion == 'shipping_cost') continue; ?>
							...
							<?php foreach (array('tax_class', 'total_value') as $criterion) { ?>
								...
								<span class="<?php echo $criterion; ?>-html" <?php if ($rule_type != $criterion) echo 'style="display: none"'; ?>>
									...
									<select class="form-control" name="<?php echo $key . '_' . $rule; ?>_value" data-help='<?php echo ${'help_'.$criterion}; ?>'>
										<option value=""><?php echo $standard_select; ?></option>
										<?php foreach (${$criterion.'_array'} as $value) { ?>
											<option value="<?php echo $value[$criterion.'_id']; ?>" <?php if ($rule_value == $value[$criterion.'_id']) echo 'selected="selected"'; ?>><?php echo $value['name']; ?></option>
										<?php } ?>
									</select>
								</span>
							<?php } ?>
							...
							<?php foreach (array('city', 'distance', 'postcode', 'shipping_rate') as $criterion) { ?>
								<?php if ($criterion == 'shipping_rate' && !in_array($criterion, $rule_options['order_criteria'])) continue; ?>
								<span class="<?php echo $criterion; ?>-html" <?php if ($rule_type != $criterion) echo 'style="display: none"'; ?>>
									<select class="form-control" name="<?php echo $key . '_' . $rule; ?>_comparison">
										<option value=""><?php echo $standard_select; ?></option>
										<option value="is" <?php if ($rule_comparison == 'is') echo 'selected="selected"'; ?>><?php echo $text_is; ?></option>
							...
							<?php foreach (array_unique(array_merge(array('geo_zone'), $rule_options['order_criteria'])) as $criterion) { ?>
					...
					<?php } ?>
						<div class="product-group-scrollbox">
							...
							<?php foreach ($scrollbox_list as $saved_value => $saved_key) { ?>
								<div><a class="btn btn-danger btn-xs" data-help='<?php echo $text_delete; ?>' onclick="removeRow($(this).parent())"><i class="fa fa-trash-o fa-lg"></i></a>
