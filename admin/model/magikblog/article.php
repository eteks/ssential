...
class ModelMagikblogArticle extends Model {
	...
	public function getBlogarticlekeyword($blog_id) {
		  ...
		  $query = $this->db->query("SELECT DISTINCT keyword FROM " . DB_PREFIX . "url_alias m WHERE query = 'mgkblogarticle_id=" . (int)$blog_id . "' ");
	      return $query->row;
	}
	...
	public function addArticle($data) {
		...
		foreach ($data['article_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "magikblog_description SET blog_id = '" . (int)$blog_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "',
		...
		if (isset($data['related_product'])) {
			foreach ($data['related_product'] as $product_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "magikblog_related_product SET blog_id = '" . (int)$blog_id . "', related_product_id = '" . (int)$product_id . "'");
			}
		}
		...
		if (isset($data['related_article'])) {
			foreach ($data['related_article'] as $related_blog_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "magikblog_related_article SET blog_id = '" . (int)$blog_id . "', related_blog_id = '" . (int)$related_blog_id . "'");
			}
		}
		...
		if ($data['keyword']) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'mgkblogarticle_id=" . (int)$blog_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
		}
	...
	public function editArticle($blog_id, $data){ 
		...
		$this->db->query("UPDATE " . DB_PREFIX . "magikblog SET author = '" . $this->db->escape($data['author']) . "',publish_date = '" . $this->db->escape($data['publishdate']) . "', status = '" . (int)$data['status'] . "', display_order = '" . (int)$data['display_order'] . "' WHERE blog_id = '" . (int)$blog_id . "'");
		if (isset($data['image'])) {
			$this->db->query("UPDATE " . DB_PREFIX . "magikblog SET image = '" . $this->db->escape($data['image']) . "' WHERE blog_id = '" . (int)$blog_id . "'");
		}
		$this->db->query("DELETE FROM " . DB_PREFIX . "magikblog_description WHERE blog_id = '" . (int)$blog_id . "'");
		foreach ($data['article_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "magikblog_description SET blog_id = '" . (int)$blog_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "',title_relatedproducts = '" . $this->db->escape($value['title_relatedproducts']) . "',title_relatedarticles = '" . $this->db->escape($value['title_relatedarticles']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "',meta_description = '" . $this->db->escape($value['meta_description']) . "',meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "',tags = '" . $this->db->escape($value['tags']) . "'");
		}
		...
		if (isset($data['article_category'])) {		
			foreach ($data['article_category'] as $category_id) {
				...
				//related products
	$this->db->query("DELETE FROM " . DB_PREFIX . "magikblog_related_product WHERE blog_id = '" . (int)$blog_id . "'");
	if (isset($data['related_product'])) {		
	foreach ($data['related_product'] as $product_id) {
	$this->db->query("INSERT INTO " . DB_PREFIX . "magikblog_related_product SET blog_id = '" . (int)$blog_id . "', related_product_id = '" . (int)$product_id . "'");
	}
	...
	//related articles
	$this->db->query("DELETE FROM " . DB_PREFIX . "magikblog_related_article WHERE blog_id = '" . (int)$blog_id . "'");
	if (isset($data['related_article'])) {		
	foreach ($data['related_article'] as $related_blog_id) {
	$this->db->query("INSERT INTO " . DB_PREFIX . "magikblog_related_article SET blog_id = '" . (int)$blog_id . "', related_blog_id = '" . (int)$related_blog_id . "'");
	}
	...
	//keyword
	$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'mgkblogarticle_id=" . (int)$blog_id . "'");
	...
	public function deleteArticle($blog_id){
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "magikblog_related_product WHERE blog_id = '" . (int)$blog_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "magikblog_related_article WHERE blog_id = '" . (int)$blog_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "magikblog_related_article WHERE related_blog_id = '" . (int)$blog_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = 'mgkblogarticle_id=" . (int)$blog_id . "'");
	}
	...
	$checkBlogTable_rp = "SHOW TABLES LIKE '".DB_PREFIX."magikblog_related_product' ";
	$queryBlog_rp=$this->db->query($checkBlogTable_rp);
	if($queryBlog_rp->num_rows!=0){	
	
	$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "magikblog_related_product WHERE blog_id = '" . (int)$blog_id . "'");
	foreach ($query->rows as $result) {
	...
	$checkBlogTable_ra = "SHOW TABLES LIKE '".DB_PREFIX."magikblog_related_article' ";
	$queryBlog_ra=$this->db->query($checkBlogTable_ra);
	if($queryBlog_ra->num_rows!=0){	
	
	$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "magikblog_related_article WHERE blog_id = '" . (int)$blog_id . "'");
	foreach ($query->rows as $result) {
