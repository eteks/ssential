...
class ModelExtensionModuleCustomProductTab extends Model {
	...
	public function createtableCustomTabs() {
				...
				$checkcustomTable = "SHOW TABLES LIKE '".DB_PREFIX."magik_productcustomtab' ";
				$queryCustom=$this->db->query($checkcustomTable);
				if($queryCustom->num_rows==0){
				    $createCustomTable = "
					CREATE TABLE " . DB_PREFIX . "magik_productcustomtab (
		...
		public function getCustomTabDescriptions($product_id) {
			...
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "magik_productcustomtab mpct WHERE mpct.product_id = '" . (int)$product_id . "'");
		...
		{
			$this->db->query("DELETE from " . DB_PREFIX . "magik_productcustomtab where  product_id = '" . (int)$data['product_id'] . "'");
			if(isset($data['custom_product_tab']))			
			{
			
			$this->db->query("INSERT INTO " . DB_PREFIX . "magik_productcustomtab SET product_id = '" . (int)$data['product_id'] . "', `value` = '" . $this->db->escape(json_encode($data)) . "' ");
			}
		}
		public function uninstallProductcustomtabs($data)
		{
			
				$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "magik_productcustomtab`");
