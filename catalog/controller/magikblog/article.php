...
class ControllerMagikblogArticle extends Controller {
	...
	public function index() {
		...
		$this->document->addStyle('catalog/view/theme/'.$this->config->get('theme_default_directory').'/stylesheet/magikblog.css');
		...
		$data['categoryname_status'] = $this->config->get('magikblog_allow_categoryname');
		...
		foreach ($results as $result) {
			$categoryName = $this->model_magikblog_article->getcategoryNamebyArticle($result['blog_id']);
			$articletags  = array();
		      if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], 915, 420);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', 915, 420);
				}

				if($result['tags']){
					$mgktags = explode( ",",$result['tags']);
					foreach( $mgktags as $tag ){
					$articletags[trim($tag)] = $this->url->link( 'magikblog/search','mgktag='.trim($tag) );
					}
				}
				else {
					$articletags=array();
	...
	public function view() {
		...
		$data['categoryname_status'] = $this->config->get('magikblog_allow_categoryname');
		$data['sociallinks_status'] = $this->config->get('magikblog_allow_sociallinks');
		...
		$data['products'] = array();
		$results = $this->model_magikblog_article->getRelatedProduct($this->request->get['mgkblogarticle_id']);	
		...
		$ra_results = $this->model_magikblog_article->getRelatedArticle($this->request->get['mgkblogarticle_id']);	
		if($ra_results) {

		foreach ($ra_results as $result) {
			$comment_total = $this->model_magikblog_article->getTotalCommentsByArticleId($result['blog_id']);
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], 915, 420);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png',915, 420);
				...
				'author'	=> $result['author'],
				'publish_date' => $result['publish_date'],
				'description' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get('magikblog_homepage_descriptionlimit')) . '..',
				'href'     => $this->url->link('magikblog/article/view', 'mgkblogarticle_id=' . $result['blog_id']),
		...
		$this->response->setOutput($this->load->view('magikblog/article_view', $data));
	...
	public function review() {
		...
		$this->response->setOutput($this->load->view('magikblog/review', $data));
