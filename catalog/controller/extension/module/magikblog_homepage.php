...
class ControllerExtensionModuleMagikblogHomepage extends Controller {
	...
	public function index() {
		...
		$this->language->load('extension/module/magikblog_homepage');
		...
		if($this->config->get('magikblog_homepage_header_text')!=''){
		$data['header_text'] = $this->config->get('magikblog_homepage_header_text');
		}else {
			$data['header_text']='';
		}
		...
		foreach ($postList as $post_List) {
			$comment_total = $this->model_magikblog_article->getTotalCommentsByArticleId($post_List['blog_id']);
			if($data['resizeimage']){
			    if ($post_List['image']) {
		...
		return $this->load->view('extension/module/magikblog_homepage', $data);
